...
def parse_subpattern(pattern, start, expect_close=False, next_group=[1]):
    ...
    while i < n:
        if pattern[i:i+3] == "(?=":
            sub_alts, new_i, next_group = parse_subpattern(pattern, i + 3, True, next_group)
            sub_tokens = sub_alts[0] if len(sub_alts) == 1 else [("OR", sub_alts)]
            current_alt.append(("LOOKAHEAD_POS", sub_tokens))
            i = new_i
            continue
        elif pattern[i:i+4] == "(?!":
            sub_alts, new_i, next_group = parse_subpattern(pattern, i + 4, True, next_group)
            sub_tokens = sub_alts[0] if len(sub_alts) == 1 else [("OR", sub_alts)]
        ...
        if c == "\\" and i + 1 < n:
            ...
            next_group[0] += 1
            sub_alts, new_i, next_group = parse_subpattern(pattern, i + 1, True, next_group)
            sub_tokens = sub_alts[0] if len(sub_alts) == 1 else [("OR", sub_alts)]
...
def tokenize(pattern):
    next_group = [1]
    alts, _, _ = parse_subpattern(pattern, 0, False, next_group)
...
def match_here(tokens, s, idx, captures, open_groups):
    ...
    if ttype == "START" and idx != 0:
        return None
    if ttype == "END" and idx != len(s):
        return None
    if ttype in {"LITERAL", "DOT", "DIGIT", "WORD", "CLASS", "NEG_CLASS"}:
        if idx >= len(s) or not match_token((ttype, val), s[idx]):
            return None
        return match_here(tokens[1:], s, idx + 1, captures, open_groups)
    if ttype == "BACKREF":
        group_num = val
        if group_num in captures:
            start, end = captures[group_num]
            frag = s[start:end]
            if s[idx:idx + len(frag)] != frag:
                return None
            return match_here(tokens[1:], s, idx + len(frag), captures, open_groups)
        elif group_num in open_groups:
            start = open_groups[group_num]
            frag = s[start:idx]
            if s[idx:idx + len(frag)] != frag:
                return None
            return match_here(tokens[1:], s, idx + len(frag), captures, open_groups)
    ...
    if ttype == "CAPTURE":
        ...
        if sub_res is None:
            return None
        pos, sub_caps = sub_res
        new_caps = sub_caps.copy()
        new_caps[group_id] = (idx, pos)
        new_open.pop(group_id, None)
        return match_here(tokens[1:], s, pos, new_caps, new_open)
    if ttype == "OR":
        for alt in val:
            alt_res = match_here(alt, s, idx, captures, open_groups)
            if alt_res:
                rest = match_here(tokens[1:], s, alt_res[0], alt_res[1], open_groups)
                if rest:
                    return rest
        return None
    if ttype == "PLUS":
        inner = val
        res = match_here([inner], s, idx, captures, open_groups)
        if not res:
            return None
        pos, caps = res
        stack = [(pos, caps)]
        while True:
            res = match_here([inner], s, pos, caps, open_groups)
            if not res:
                break
            pos, caps = res
    ...
    if ttype == "QUESTION":
        ...
        res = match_here([inner], s, idx, captures, open_groups)
        if res:
            rest = match_here(tokens[1:], s, res[0], res[1], open_groups)
            if rest:
                return rest
