...
def main():
    ...
    if not args or "-E" not in args:
        print("Usage: ./your_program.sh [-r] -E <pattern> [file_or_dir ...]", file=sys.stderr)
        sys.exit(2)
    ...
    try:
        ...
        print("Missing -E flag", file=sys.stderr)
        sys.exit(2)

    if e_index + 1 >= len(args):
        print("Pattern missing after -E", file=sys.stderr)
        sys.exit(2)

    pattern = args[e_index + 1]
    files = args[e_index + 2:]
